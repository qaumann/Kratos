	...
	private:
								  ...
								  unsigned int &maxId)
		{
			...
			for (unsigned int nn = 0; nn < NewPositions.size(); nn++)
			{
				...
				Node<3>::Pointer pnode = mrModelPart.CreateNewNode(id, x, y, z); //this has been added
				//pnode = Kratos::make_intrusive<Node<3>>(id, x, y, z); //this has been commented
			...
			for (std::vector<Node<3>::Pointer>::iterator it = list_of_new_nodes.begin(); it != list_of_new_nodes.end(); it++)
			{
				...
				//mrModelPart.Nodes().push_back(*(it));//this has been commented
			}
